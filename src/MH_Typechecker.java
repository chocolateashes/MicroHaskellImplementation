...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
    	if (exp.isBOOLEAN()){
    		...
    		MH_TYPE e1 = computeType(exp.first(), env);
    		MH_TYPE e2 = computeType(exp.second(), env);    		
    		

    		if (e1.equals(IntegerType)) {
    			if (e2.equals(IntegerType)) {
    				...
    				throw new TypeError ("Inputs need to be Integers");
    			}
    		}
    		else {
    			throw new TypeError ("Inputs need to be Integers");
    		...
    		MH_TYPE e1 = computeType(exp.first(), env);
    		MH_TYPE e2 = computeType(exp.second(), env);
    		MH_TYPE e3 = computeType(exp.third(), env);
    		
    		if (e1.equals(BoolType)) {
    			if (e2.equals(BoolType) && e3.equals(BoolType)) {
    					return BoolType;
    				}
    			else if (e2.equals(IntegerType) && e3.equals(IntegerType)){
    					return IntegerType;}
    			else{
    				throw new TypeError ("Mismatch types");}
    		}
    		else {
    			throw new TypeError ("First expression must be boolean");
    		}
    	}
    		...
    		if (e1.isArrow()) {
    			if (e1.left().equals(e2)) {
    				return e1.right();
    			}
    			else throw new TypeError ("Types mismatched");
    		}
